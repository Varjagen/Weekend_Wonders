...
const makeDefaultState = () => {
  ...
  return {
    ...
    //   } }
    tokenGroups: {},
    // v7.6: spectator vision scope. DM-controlled, broadcast to all.
    //   'lights' — spectators see only visible light sources (e.g.
    //     a torch's lightRadius). They cannot see through PC
    //     darkvision. Most cinematic — they see what an outside
    //     observer would see.
    //   'all'    — spectators see everything the party collectively
    //     sees (lights + every PC's darkvision range). Best for
    //     observers who want full battlefield awareness.
    spectatorVisionMode: 'lights',
  };
};
...
function migrateState(raw) {
  ...
  // v7.6: spectator vision scope
  if (state.spectatorVisionMode !== 'lights' && state.spectatorVisionMode !== 'all') {
    state.spectatorVisionMode = 'lights';
  }
...
function reducer(state, action) {
  switch (action.type) {
    ...
    // v7.6: Short rest. D&D 5e semantics: doesn't auto-restore HP
    // (that's spent hit dice), but does clear short-term conditions
    // and reset death saves. We give a small, predictable HP nudge
    // (1/4 of max, rounded up) so the rest button does something
    // visible without overstepping into "use a hit die for me"
    // territory. The DM can still hand-edit HP precisely.
    //
    // Conditions cleared on a short rest: a narrower set than long
    // rest — only conditions that lift with brief reprieve (Stunned,
    // Prone, Grappled, Incapacitated, Frightened). Things that need
    // longer recovery (Exhausted, Poisoned, Paralyzed, etc.) stay.
    case 'SHORT_REST': {
      const targetIds = Array.isArray(action.entityIds)
        ? action.entityIds
        : Object.values(state.entities).filter(e => e.type === 'PC' || e.type === 'Familiar').map(e => e.id);
      const SHORT_CLEARED = new Set(['Stunned', 'Prone', 'Grappled', 'Incapacitated', 'Frightened']);
      const entities = { ...state.entities };
      for (const id of targetIds) {
        const e = entities[id];
        if (!e) continue;
        const max = e.hp?.max || 0;
        const curr = e.hp?.current || 0;
        // 1/4 max HP nudge, capped at max; only if alive
        const bump = (curr > 0 && max > 0) ? Math.min(max - curr, Math.ceil(max / 4)) : 0;
        entities[id] = {
          ...e,
          hp: { ...e.hp, current: curr + bump },
          conditions: e.conditions.filter(c => !SHORT_CLEARED.has(c)),
          deathSaves: { successes: 0, failures: 0 },
        };
      }
      return { ...state, entities };
    ...
    // v7.6: Ghost-claim sweep. Drop claim records whose peerId hasn't
    // appeared in the live connection list for too long. Avoids
    // accumulating ghost players in the party panel after reload
    // crashes / browser closes that didn't finish CLAIM_MIGRATE.
    //
    // The DM dispatches this periodically with the current set of
    // connected peerIds + a grace period (in ms). Any claim record
    // whose peerId is NOT in livePeerIds AND whose lastSeenTs is
    // older than the cutoff is dropped, UNLESS the claim has a
    // playerId — those are kept around to allow reconnection from a
    // new peerId via CLAIM_MIGRATE. Keep claims that own a PC slot
    // even without playerId for one full sweep cycle to give slow
    // reconnects a chance.
    // v7.6: DM control for what spectators see at night / in dark
    // maps. Two modes (see makeDefaultState comment for semantics).
    case 'SPECTATOR_VISION_SET': {
      const mode = action.mode === 'all' ? 'all' : 'lights';
      if (state.spectatorVisionMode === mode) return state;
      return { ...state, spectatorVisionMode: mode };
    }

    case 'CLAIMS_GHOST_SWEEP': {
      const { livePeerIds, cutoffMs = 30000 } = action;
      ...
      for (const [pid, c] of Object.entries(claims)) {
        if (live.has(pid)) {
          // Live peer — keep with a fresh lastSeenTs stamp
          const updated = { ...c, lastSeenTs: now };
          if (updated.lastSeenTs !== c.lastSeenTs) changed = true;
          next[pid] = updated;
          continue;
        }
        // Not currently connected. Decide whether to keep.
        if (c.playerId) {
          // Has a stable playerId — keep so a reconnect can migrate
          next[pid] = c;
          continue;
        }
        const lastSeen = c.lastSeenTs || c.createdTs || 0;
        if (lastSeen === 0) {
          // No timestamp — give it a fresh one rather than nuking
          next[pid] = { ...c, lastSeenTs: now };
          changed = true;
          continue;
        }
...
class SyncManager {
  ...
  async hostSession(roomCode) {
    ...
    try {
      ...
      // v7.6: PeerJS fires 'disconnected' when the underlying socket to
      // the signaling server drops (which happens when a browser tab is
      // backgrounded for too long — Chrome / Safari throttle inactive
      // tabs hard, killing keepalives). The peer object itself stays
      // valid; calling `peer.reconnect()` re-establishes the signaling
      // socket WITHOUT needing all clients to re-join.
      this.peer.on('disconnected', () => {
        console.log(`[plagues-call] DM peer disconnected — attempting reconnect`);
        this.setStatus('reconnecting');
        this._scheduleReconnect();
      });
      this.peer.on('error', (err) => {
        if (err.type === 'unavailable-id') {
          // v7.6: This is what happens if we try to host with a code
          // that's already taken — possibly by a stale ghost peer of
          // ourselves. Don't surface as a hard error immediately; try
          // a forced reconnect first.
          console.log(`[plagues-call] DM unavailable-id, attempting reclaim of ${roomCode}`);
          ...
          // Recoverable — schedule reconnect rather than declaring dead
          console.log(`[plagues-call] DM network error: ${err.type} — will reconnect`);
          this.setStatus('reconnecting');
          this._scheduleReconnect();
        } else {
          this.setStatus('error');
  ...
  // v7.6: Reconnect with backoff. Both DM and player paths converge
  // here. PeerJS's `peer.reconnect()` reuses the existing peer id; if
  // the peer was destroyed entirely we recreate. Backoff: 1s, 2s, 4s,
  // 8s, then every 8s. Cancellable by destroy().
    ...
    this._reconnectTimer = setTimeout(() => {
      ...
      try {
        if (this.peer && !this.peer.destroyed && this.peer.disconnected) {
          this.peer.reconnect();
          // Will fire 'open' on success; if it fires another error/disconnect,
          // _scheduleReconnect will recurse with attempt+1
          setTimeout(() => {
            if (!this._destroyed && this.peer?.disconnected) {
              this._scheduleReconnect(attempt + 1);
            }
          }, delay + 2000);
        } else if (!this.peer || this.peer.destroyed) {
          // Full re-host / re-join needed
          if (this.mode === 'dm') {
            this.hostSession(this.roomCode);
          } else if (this.mode === 'player') {
            this.joinSession(this.roomCode, this.playerId, this.playerName);
          }
        }
      } catch (err) {
        console.log(`[plagues-call] reconnect attempt ${attempt} failed: ${err?.message}`);
        this._scheduleReconnect(attempt + 1);
      }
    }, delay);
  }
  async joinSession(roomCode, playerId, playerName) {
    ...
    try {
      ...
      this.peer.on('open', (id) => {
        ...
        conn.on('close', () => {
          // v7.6: if the connection to the DM closed but our peer is
          // still alive, the DM's room may have hiccupped. Schedule
          // a reconnect rather than going offline forever.
          if (!this._destroyed) {
            console.log(`[plagues-call] player connection to DM closed — will retry`);
            this.setStatus('reconnecting');
            this._scheduleReconnect();
          } else {
            this.setStatus('offline');
      ...
      // v7.6: same disconnect-and-reconnect logic as DM path.
      this.peer.on('disconnected', () => {
        console.log(`[plagues-call] player peer disconnected — attempting reconnect`);
        this.setStatus('reconnecting');
        this._scheduleReconnect();
      });
      this.peer.on('error', (err) => {
        if (err.type === 'network' || err.type === 'socket-error' || err.type === 'server-error') {
          console.log(`[plagues-call] player network error: ${err.type} — will reconnect`);
          this.setStatus('reconnecting');
          this._scheduleReconnect();
        } else if (err.type === 'peer-unavailable') {
          // DM not online (yet?). Show meaningful error, but keep retrying.
          this.setStatus('reconnecting');
          this.onError?.('Could not reach the DM. Will keep trying.');
          this._scheduleReconnect();
        } else {
          this.setStatus('error');
...
function filterStateForPlayer(state, peerId) {
  ...
  // v7.6: spectators have no PC of their own, so the regular vision-
  // cutoff would strip every token at night. The DM-controlled
  // `spectatorVisionMode` decides what they get to see:
  //   'lights' — they see what visible light sources illuminate (any
  //              entity with a lightRadius > 0). Useful for cinematic
  //              "uninvolved observer" feel.
  //   'all'    — they see what the WHOLE party collectively sees,
  //              i.e. every PC's darkvision range plus all light
  //              sources. Useful for observers who need full
  //              battlefield awareness.
  // To make the existing visibility cutoff work without rewriting it,
  // we synthesize a `visionAugmentIds` set: for spectators only, this
  // includes the entityIds of the relevant vision sources. Those
  // entities are NOT "owned" (the spectator can't move them) but ARE
  // treated as vision sources by the visibility cutoff downstream.
  const isSpectator = !!claim.spectator && !claim.pc && (!claim.familiars || claim.familiars.length === 0);
  const spectatorMode = state.spectatorVisionMode === 'all' ? 'all' : 'lights';
  const visionAugmentIds = new Set();
  if (isSpectator) {
    for (const [eid, ent] of Object.entries(state.entities)) {
      if (!ent) continue;
      if (spectatorMode === 'all') {
        // Whole-party mode: any PC or Familiar contributes their
        // darkvision range AND lights.
        if (ent.type === 'PC' || ent.type === 'Familiar') {
          visionAugmentIds.add(eid);
        } else if ((ent.lightRadius || 0) > 0) {
          // Non-party light sources too
          visionAugmentIds.add(eid);
        }
      } else {
        // Lights-only mode: any entity with a lightRadius. Includes
        // a torch-bearing PC (light source counts) but not their
        // darkvision.
        if ((ent.lightRadius || 0) > 0) visionAugmentIds.add(eid);
      }
    }
  }
  ...
  if (visionActive) {
    // v7.6: For spectators, swap in the synthetic "augmented" ID set
    // (lights only, or full party vision, depending on DM setting).
    // For regular players, use ownedIds as before.
    const sourceIds = isSpectator ? visionAugmentIds : ownedIds;
    // Compute the vision sources this player has on this map.
    const sources = computePlayerVisionSources(state, effectiveMapId, sourceIds, tod, mapAlwaysDark);
    ...
    for (const [k, t] of Object.entries(visibleTokens)) {
      ...
      // v7.6: PCs and Familiars stay visible to spectators in 'all'
      // mode (they're already in visionAugmentIds). In 'lights' mode,
      // PCs holding light sources are visible (they're in
      // visionAugmentIds via lightRadius). Spectators without a torch
      // in range during 'lights' mode see nothing of unlit PCs — by
      // design.
      let visible = false;
...
function SettingsModal({
  settings, onChange, onClose, mode, mapScale, onMapScaleChange,
  // v7.6: import/export migrated here from the topbar
  onExportSession, onImportSession,
  // v7.6: DM-only — spectator vision scope. 'lights' | 'all' | undefined
  spectatorVisionMode, onSpectatorVisionChange,
}) {
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal slide-up" style={{ maxWidth: 460 }}>
        ...
        <div className="float-panel-body">
          ...
          {/* v7.6: Spectator vision scope — DM only. Controls what
              spectators can see at night / in dark maps. */}
          {mode === 'dm' && spectatorVisionMode != null && onSpectatorVisionChange && (
            <div className="settings-section">
              <label className="settings-label">
                Spectator Vision <span className="settings-label-sub">— what observers see in the dark</span>
              </label>
              <div className="theme-switch">
                <button
                  className={`theme-option ${spectatorVisionMode === 'lights' ? 'active' : ''}`}
                  onClick={() => onSpectatorVisionChange('lights')}
                >
                  <span className="theme-sub" style={{ marginTop: 0, fontSize: 18 }}>🕯</span>
                  <span>Lights only</span>
                  <span className="theme-sub">Cinematic — sees what light reveals</span>
                </button>
                <button
                  className={`theme-option ${spectatorVisionMode === 'all' ? 'active' : ''}`}
                  onClick={() => onSpectatorVisionChange('all')}
                >
                  <span className="theme-sub" style={{ marginTop: 0, fontSize: 18 }}>👁</span>
                  <span>Party vision</span>
                  <span className="theme-sub">All PCs' darkvision + lights</span>
                </button>
              </div>
              <div className="settings-hint">
                Spectators don't have a character, so without this they'd see nothing
                at night. Pick "Lights only" for a cinematic uninvolved-observer feel,
                or "Party vision" if your spectators want full battlefield awareness.
              </div>
            </div>
          )}

          {/* v7.6: Import / Export — DM only. Moved here from the
              topbar so the topbar stays focused on play-time controls. */}
          {mode === 'dm' && onExportSession && onImportSession && (
            <div className="settings-section">
              <label className="settings-label">
                Session Data <span className="settings-label-sub">— full state JSON</span>
              </label>
              <div style={{ display: 'flex', gap: 6 }}>
                <button className="btn" style={{ flex: 1 }} onClick={() => { onExportSession(); onClose(); }}>
                  ⇩ Export to JSON
                </button>
                <button className="btn" style={{ flex: 1 }} onClick={() => { onImportSession(); onClose(); }}>
                  ⇧ Import from JSON
                </button>
              </div>
              <div className="settings-hint">
                Export saves a snapshot of the entire campaign (entities, maps, hazards,
                drawings, claims, presets). Import replaces the current state — share
                with another DM or use as a manual backup.
              </div>
            </div>
          )}
...
// ====================================================================
// RESIZABLE SIDEBAR  (v7.6)
// ====================================================================
// Wraps a sidebar (bestiary / party / revealed-monsters) with:
//   1. A drag-handle on the inner edge that lets the user adjust width
//   2. A collapse button (▸ / ◂) that shrinks the panel to a thin strip
//      revealing only the toggle button — clicking the strip expands
//      the panel back to its previous width
// Width and collapse state persist in localStorage keyed by `id`.
//
// Props:
//   id           — unique key for localStorage (e.g. 'sidebar-bestiary')
//   side         — 'left' | 'right'; controls handle position + collapse direction
//   defaultWidth — initial width if no localStorage value yet
//   minWidth     — minimum allowed width during resize (default 200)
//   maxWidth     — maximum allowed width (default 600)
//   className    — extra classes on the outer wrapper
//   children     — sidebar content
function ResizableSidebar({
  id, side = 'left', defaultWidth = 280,
  minWidth = 200, maxWidth = 600,
  className = '', children,
}) {
  const widthKey = `sidebar-w-${id}`;
  const collapsedKey = `sidebar-c-${id}`;
  // Initial values from localStorage, with sensible fallbacks.
  ...
  // Persist on change.
  useEffect(() => {
    try { localStorage.setItem(widthKey, String(Math.round(width))); } catch {}
  }, [width, widthKey]);
  useEffect(() => {
    try { localStorage.setItem(collapsedKey, collapsed ? '1' : '0'); } catch {}
  }, [collapsed, collapsedKey]);

  // Drag-resize via pointer events. The handle is a 6px-wide vertical
  // strip on the INNER edge of the sidebar (right edge for left
  // sidebars, left edge for right sidebars). Pointer-events-only —
  // touch & mouse both produce pointer events on modern browsers.
  const startResize = (e) => {
    if (collapsed) return;
    e.preventDefault();
  ...
  // Collapse arrow direction: left sidebar collapsed shows ▸ to expand
  // (rightward); right sidebar collapsed shows ◂ to expand (leftward).
  const collapseArrow = collapsed
    ? (side === 'left' ? '▸' : '◂')
    : (side === 'left' ? '◂' : '▸');

  // While collapsed, the entire collapsed strip is clickable to expand
  // (improves discoverability over hunting for a tiny button).
  const onCollapsedStripClick = (e) => {
    if (e.target.closest('button')) return; // let the button itself handle
    setCollapsed(false);
  };

  return (
    <div
      className={`sidebar resizable-sidebar ${side} ${collapsed ? 'collapsed' : ''} ${className}`.trim()}
      style={{ width: collapsed ? 24 : width }}
      onClick={collapsed ? onCollapsedStripClick : undefined}
    >
      {/* Collapse / expand button. Always rendered, positioned at the
          inner edge (top corner) so it stays reachable in either state. */}
      <button
      ...
      {/* Sidebar content — hidden via CSS overflow when collapsed. */}
      {!collapsed && (
        <div className="sidebar-inner">
          {children}
        </div>
      )}
      {/* Resize handle. Hidden when collapsed (no need to drag a 24px
          strip). Pointer-down starts the resize gesture. */}
      {!collapsed && (
        <div
...
// MEASURE MENU  (v7.6)
// ====================================================================
// Small dropdown for picking the measure mode (line vs radius). The
// active measure-mode label appears in the trigger.
function MeasureMenu({ measureMode, setMeasureMode }) {
  ...
  const label = measureMode === 'line' ? '↔ Line'
              : measureMode === 'radius' ? '⊚ Radius'
              : '↔ Measure';
  return (
    <div className="tools-menu-wrap" ref={wrapRef}>
      <button className={`btn ${measureMode ? 'active' : ''} ${open ? 'active' : ''}`}
        onClick={() => setOpen(o => !o)}
        ...
        <div className="tools-menu-pop">
          <div className="tools-section">
            <button className={`tools-item ${measureMode === 'line' ? 'active' : ''}`}
              onClick={choose('line')}>
              ↔ Line <span className="tools-hint">point-to-point distance</span>
            </button>
            <button className={`tools-item ${measureMode === 'radius' ? 'active' : ''}`}
              onClick={choose('radius')}>
              ⊚ Radius <span className="tools-hint">circle from a center</span>
            </button>
            {measureMode && (
              <button className="tools-item ghost"
                onClick={() => { setMeasureMode(null); setOpen(false); }}>
                ✕ Stop measuring
              </button>
            )}
          </div>
...
// ====================================================================
// SHAPES & AREAS MENU  (v7.6)
// ====================================================================
// Replaces the Shapes & Areas section of the consolidated tools menu.
// Picks the current block-shape mode (rect / freeform / circle / erase)
// and offers a shortcut into the hazards palette.
function ShapesMenu({
  ...
  const [open, setOpen] = useState(false);
  const wrapRef = useRef(null);
  const anyActive = placingBlock || placingFreeBlock || placingCircleBlock || erasingBlock;
  useEffect(() => {
    if (!open) return;
    const onDoc = (e) => { if (!wrapRef.current?.contains(e.target)) setOpen(false); };
    const onEsc = (e) => { if (e.key === 'Escape') setOpen(false); };
  ...
  // Picking one mode disables the others
  const choose = (which) => () => {
    setOpen(false);
    setPlacingBlock(which === 'rect' ? !placingBlock : false);
    setPlacingFreeBlock(which === 'free' ? !placingFreeBlock : false);
    setPlacingCircleBlock(which === 'circle' ? !placingCircleBlock : false);
    setErasingBlock(which === 'erase' ? !erasingBlock : false);
  };
  const label = placingBlock ? '◼ Rect…'
              : placingFreeBlock ? '✎ Free…'
              : placingCircleBlock ? '⬤ Circle…'
              : erasingBlock ? '✕ Erasing…'
              : '◧ Shapes';
  return (
    <div className="tools-menu-wrap" ref={wrapRef}>
      <button className={`btn ${anyActive ? 'active' : ''} ${open ? 'active' : ''}`}
        onClick={() => setOpen(o => !o)}
        title="Shapes & Areas — block zones, hazards">
        {label} ▾
      </button>
      {open && (
        <div className="tools-menu-pop">
          <div className="tools-section">
            <div className="tools-section-title">Block Zones</div>
            <button className={`tools-item ${placingBlock ? 'active' : ''}`}
              onClick={choose('rect')}>
              ◼ Rectangle <span className="tools-hint">click-drag</span>
            </button>
            <button className={`tools-item ${placingFreeBlock ? 'active' : ''}`}
              onClick={choose('free')}>
              ✎ Freeform <span className="tools-hint">draw a polygon</span>
            </button>
            <button className={`tools-item ${placingCircleBlock ? 'active' : ''}`}
              onClick={choose('circle')}>
              ⬤ Circle <span className="tools-hint">drag from center</span>
            </button>
          ...
          <div className="tools-section">
            <div className="tools-section-title">Hazards</div>
...
// REST MENU  (v7.6)
// ====================================================================
// Small dropdown that lets the DM choose between Long Rest and Short
// Rest. Replaces the standalone Long Rest button. Closes on outside
// click, on Escape, and after either rest action fires.
function RestMenu({ onLongRest, onShortRest }) {
  const [open, setOpen] = useState(false);
  const wrapRef = useRef(null);
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="tools-menu-wrap" ref={wrapRef}>
      <button className={`btn ${open ? 'active' : ''}`} onClick={() => setOpen(o => !o)}
        title="Rest the party (long or short)">
        ⛭ Rest ▾
      </button>
      {open && (
        <div className="tools-menu-pop">
          <div className="tools-section">
            <button className="tools-item" onClick={choose(onLongRest)}>
              ⛭ Long Rest
              <span className="tools-hint">full HP, clear most conditions</span>
            </button>
            <button className="tools-item" onClick={choose(onShortRest)}>
              ⛣ Short Rest
              <span className="tools-hint">¼ HP, clear short-term conditions</span>
            </button>
          </div>
        </div>
      )}
...
// ====================================================================
// ====================================================================
// PUSH VIEW PANEL  (v7.6)
// ====================================================================
// Extracted from DMWorldPanel into its own dedicated panel + toolbar
// button. Lets the DM:
//   - push the current map to ALL players (global override)
//   - push specific maps to individual players (per-peer override)
//   - release any/all pushes
// Uses the same FORCED_VIEW + FORCED_VIEW_PEER_SET reducer actions as
// before; only the UI separation is new.
function PushViewPanel({ state, dispatch, onClose, toast }) {
  ...
  return (
    <FloatPanel className="push-view-panel" style={{ right: 16, top: 80, width: 360 }}>
      <div className="float-panel-header">
        <span>⚑ Push View</span>
        <button className="close-x" onClick={onClose}>×</button>
      </div>
      <div className="float-panel-body">
        <div className="settings-hint" style={{ marginBottom: 8 }}>
          Force what map players see. Useful for cinematic reveals, combat
          transitions, or making sure everyone is looking at the right scene.
        </div>

        <div className="settings-section">
          <label className="settings-label">Global</label>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
            <button className={`btn sm ${state.forcedView?.mapId === currentMapId ? 'danger active' : ''}`}
              onClick={pushGlobal}>
              {state.forcedView?.mapId === currentMapId ? '⚑ Release All' : '⚑ Push current map to All'}
            </button>
            <button className="btn sm ghost" onClick={clearAllPush}>Clear all pushes</button>
          </div>
          <div className="settings-hint">
            Global push overrides per-peer. Releasing it returns each player to their own current map.
          </div>
        </div>

        <div className="settings-section">
          <label className="settings-label">Per-Player</label>
          {peers.length === 0 ? (
  ...
  return (
    <FloatPanel className="world-panel" style={{ right: 16, top: 80, width: 360 }}>
      ...
      <div className="float-panel-body">
        ...
        {/* v7.6: Push View moved to its own panel — see PushViewPanel
            and the dedicated toolbar button. Keeps the World panel
            focused on environmental controls (time of day + blocks). */}
...
function DMInterface({ state, dispatch, sync, syncStatus, peerCount, onLogout, roomCode, toast, settings, onSettingsChange, onOpenSettings, showSettings, onCloseSettings }) {
  ...
  // v7.6: Push View pulled into its own panel
  const [showPushView, setShowPushView] = useState(false);
  ...
  // v7.6: Short rest. A briefer recovery — heals 1/4 max HP per
  // character, clears short-term conditions only (Stunned, Prone,
  // Grappled, Incapacitated, Frightened). The DM can still hand-edit
  // HP for precise hit-die tracking.
  const shortRestAll = () => {
    if (!confirm('Short rest: heal 1/4 HP for all party, clear short-term conditions?')) return;
    dispatch({ type: 'SHORT_REST' });
    toast('A short reprieve. Breath returns.', 'success', 3000);
  };
  const shortRestOne = (entityId) => {
    dispatch({ type: 'SHORT_REST', entityIds: [entityId] });
    const e = state.entities[entityId];
    toast(`${e?.name || 'Character'} caught their breath.`, 'success');
  };
  ...
  return (
    <div className="app-shell">
      <div className="topbar">
        ...
        {/* v7.6: each tool category gets its own dedicated toolbar
            button. Replaces the v7 consolidated 🧰 Tools dropdown.
            Measure and Shapes need internal mode-pickers (small
            dropdowns); the rest are simple toggles or open panels. */}
        <MeasureMenu
          measureMode={measureMode}
          setMeasureMode={setMeasureMode}
        />
        <button className={`btn ${showDraw ? 'active' : ''}`}
        ...
        />
        <button className={`btn ${showGroups ? 'active' : ''}`}
          onClick={() => setShowGroups(!showGroups)}
          title="Encounter — token groups">
          ⋱ Encounter
        </button>
        <button className={`btn ${showDice ? 'active' : ''}`}
          onClick={() => setShowDice(!showDice)}
          title="Dice tray">
          🎲 Dice
        </button>
        <button className={`btn ${showSounds ? 'active' : ''}`}
          onClick={() => setShowSounds(!showSounds)}
          title="Soundboard">
          🔊 Sounds
        </button>
        <button className={`btn ${placingReminder ? 'active' : ''}`}
          onClick={() => setPlacingReminder(p => !p)}
          title="Place a reminder marker on the map">
          ⚐ Reminder
        </button>
        <button className={`btn world-btn ${showWorld ? 'active' : ''}`}
          onClick={() => setShowWorld(!showWorld)}
          title="World: time of day, block zones">
          {(() => {
        ...
        {/* v7.6: dedicated Push View button. Independent of World. */}
        <button className={`btn ${showPushView ? 'active' : ''}`}
          onClick={() => setShowPushView(!showPushView)}
          title="Push View — force what map players see">
          ⚑ Push
        </button>
        <RestMenu
          onLongRest={longRestAll}
          onShortRest={shortRestAll}
        />
        <div className="topbar-spacer" />
        {roomCode && (
          <div className="conn-status">
            <div className={`conn-dot ${syncStatus === 'live' ? 'live' : (syncStatus === 'connecting' || syncStatus === 'reconnecting') ? 'connecting' : syncStatus === 'error' ? 'error' : ''}`} />
      ...
      <div className="main">
        <ResizableSidebar id="dm-bestiary" side="left" defaultWidth={280} minWidth={200} maxWidth={520}>
          <EntitySidebar
            state={state}
            dispatch={dispatch}
      ...
      {showSettings && (
        <SettingsModal
          ...
          spectatorVisionMode={state.spectatorVisionMode || 'lights'}
          onSpectatorVisionChange={(mode) => dispatch({ type: 'SPECTATOR_VISION_SET', mode })}
        />
      )}
    </div>
  );
...
function PlayerInterface({ state, myPeerId, playerName, sync, syncStatus, onLogout, roomCode, toast, settings, onSettingsChange, onOpenSettings, showSettings, onCloseSettings }) {
  ...
  const isSpectator = !!myClaim.spectator && !claimedEntityId && claimedFamiliarIds.length === 0;
  ...
  // v7.6: For spectators we synthesize a set of "vision-source" entity
  // IDs based on the DM-controlled spectatorVisionMode. The spectator
  // doesn't OWN these entities (movement is still locked out for them
  // because ownedEntityIds is empty), but they ARE used as origin
  // points for vision sources so the vision mask actually reveals
  // something to look at.
  const spectatorVisionMode = state.spectatorVisionMode === 'all' ? 'all' : 'lights';
  const visionSourceIds = useMemo(() => {
    if (!isSpectator) return ownedEntityIds;
    const s = new Set();
    for (const [eid, ent] of Object.entries(state.entities || {})) {
      if (!ent) continue;
      if (spectatorVisionMode === 'all') {
        if (ent.type === 'PC' || ent.type === 'Familiar') s.add(eid);
        else if ((ent.lightRadius || 0) > 0) s.add(eid);
      } else {
        if ((ent.lightRadius || 0) > 0) s.add(eid);
      }
    }
    return s;
  }, [isSpectator, spectatorVisionMode, state.entities, ownedEntityIds]);
  ...
  // bonded familiars.
  // v7.6: spectators use the synthesized visionSourceIds so the vision
  // gate properly turns ON for them in the dark.
  const visionOwned = useMemo(
    () => Array.from(visionSourceIds).map(id => state.entities[id]).filter(Boolean),
    [visionSourceIds, state.entities]
  );
  ...
  // v7.6: spectators pass visionSourceIds (all PCs/lights or just lights)
  const tod = state.timeOfDay || 0;
  const mapAlwaysDark = !!currentMap?.alwaysDark;
  const playerVisionSources = useMemo(
    () => {
      const t0 = performance.now();
      const res = computePlayerVisionSources(state, currentMapId, visionSourceIds, tod, mapAlwaysDark);
  ...
  return (
    <div className="app-shell">
      <div className="topbar">
        ...
        <div className="topbar-divider" />
        <button className={`btn ${showInit ? 'active' : ''}`} onClick={() => setShowInit(!showInit)}>⚔ Initiative</button>
        {/* v7.6: Players get the same individual-button layout as the
            DM (where applicable). Players have measure, draw, dice,
            and reminder — no block tools, no soundboard, no encounter. */}
        <MeasureMenu
          measureMode={measureMode}
          setMeasureMode={setMeasureMode}
        />
      ...
      <div className="main player-view">
        <ResizableSidebar id="player-party" side="left" defaultWidth={260} minWidth={180} maxWidth={460}>
          <PartySidebar
            state={state}
        ...
        <div className={`canvas-container sick-level-${sicknessLevel} tod-${Math.round((state.timeOfDay || 0) * 10)} ${claimedEntity && claimedEntity.hp.current <= 0 ? 'downed' : ''}`}>
          ...
          {syncStatus !== 'live' && (
            <div className="canvas-overlay bottom-center">
              <div className="forced-view-banner">
                {syncStatus === 'connecting' ? 'Connecting to the table…'
                 : syncStatus === 'reconnecting' ? 'Reconnecting…'
                 : syncStatus === 'error' ? 'Connection error. Trying to reach the DM…'
                 : 'Offline'}
              </div>
            </div>
          )}
        ...
        <ResizableSidebar id="player-revealed" side="right" defaultWidth={240} minWidth={180} maxWidth={420}>
...
function Session({ auth, onLogout, settings, onSettingsChange, showSettings, onOpenSettings, onCloseSettings }) {
  ...
  useEffect(() => {
    ...
    // v7.6: when the tab returns to visibility, check whether the
    // peer is disconnected. Browsers throttle inactive tabs hard,
    // killing the signaling-server keepalive after a few minutes.
    // PeerJS surfaces this as `peer.disconnected === true` while the
    // peer object itself remains valid. Calling reconnect() re-opens
    // the signaling socket without disturbing peer connections.
    const onVisibilityReturn = () => {
      if (document.hidden) return;
      if (sync._destroyed) return;
      if (sync.peer && sync.peer.disconnected) {
        console.log(`[plagues-call] tab returned, peer disconnected — reconnecting`);
        sync._scheduleReconnect();
      }
    };
  ...
  // v7.6: Ghost-claim sweep. Every 15 seconds the DM checks for claim
  // records whose peerIds aren't currently connected and aren't keyed
  // to a stable playerId — those are leftovers from crashed players.
  // Drops them so the party panel doesn't accumulate ghosts.
  useEffect(() => {
    if (auth.mode !== 'dm' || !syncRef.current) return;
    const interval = setInterval(() => {
      dispatch({
        type: 'CLAIMS_GHOST_SWEEP',
        livePeerIds: Array.from(syncRef.current.connections.keys()),
        cutoffMs: 30000, // 30s grace
      });
    }, 15000);
