...
// TOKEN DETAIL PANEL
// ====================================================================
function TokenDetailPanel({ state, token, entity, mode, dispatch, onClose, claimedEntityId, playerActionSender, onLongRest, obfuscateHp }) {
...
function EditMySheetModal({ state, myPeerId, claim, playerActionSender, onClose, obfuscateHp }) {
  const [hpDelta, setHpDelta] = useState(0);
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal slide-up" style={{ maxWidth: 520 }}>
        ...
        <div className="float-panel-body">
          ...
          <label>HP</label>
          {obfuscateHp ? (
            <div style={{ marginBottom: 8, padding: '6px 8px', background: 'var(--bg-0)', borderRadius: 4, fontSize: 12, color: 'var(--ink-dim)', fontStyle: 'italic' }}>
              HP hidden by DM — {hpLabel(entity.hp.max > 0 ? entity.hp.current / entity.hp.max * 100 : 0).text}
            </div>
          ) : (
            <>
