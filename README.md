...
## Changelog — v7.6 (UX overhaul + connection resilience)

A wide-ranging update touching connection lifecycle, spectator mode, the toolbar, sidebar layout, and the rest workflow. Every feature was driven by a real issue you reported — none of this is speculative scope.

### Fix #1 — Connection resilience (player reload + DM tab-blur)

The two underlying bugs were independent but symptomatically similar.

**Player reload errors / ghost players:**
- A reloading player got a NEW peerId from PeerJS while their old claim record still lived under the OLD peerId in `state.claims`. Without an explicit cleanup, both records existed after reconnect — sometimes for hours.
- The `hello` envelope-driven `CLAIM_MIGRATE` already moved claims to the new peerId, but only when reconnect succeeded cleanly. Crashes that didn't fire `close` left orphans.

**DM tab-blur kills the room:**
- Browsers throttle inactive tabs hard. After a few minutes backgrounded, PeerJS's signaling-server keepalive fails. The peer fires `disconnected` (the underlying socket is dead) — but the existing code didn't listen for that event.
- All connected players' WebRTC channels then time out, requiring everyone to reload the page.

**Fix:**

1. **Auto-reconnect with backoff** in `SyncManager`. New `_scheduleReconnect(attempt)` method with 1s → 2s → 4s → 8s exponential delay. Triggered by:
   - `peer.on('disconnected')` (signaling socket lost)
   - Recoverable error types: `network`, `socket-error`, `server-error`, `peer-unavailable`
   - Player-side `conn.on('close')` while peer is alive
2. **Tab-blur recovery** via `document.addEventListener('visibilitychange', ...)` in the Setup-sync effect. When the tab returns to focus and `peer.disconnected === true`, it calls `_scheduleReconnect()`.
3. **`_destroyed` flag** in `destroy()` cancels any scheduled reconnect to avoid zombie timers.
4. **Status `'reconnecting'`** added; both DM topbar conn-dot and player overlay banner show "Reconnecting…" instead of "Offline" or "Connection lost".
5. **`CLAIMS_GHOST_SWEEP` reducer action** + 15-second DM-side interval that prunes claim records whose peerIds aren't currently connected and don't have a `playerId` (claims with a `playerId` are kept so reconnects can migrate them). Live peers get a fresh `lastSeenTs` stamp.

You should see: reload as a player → seamless rejoin, claim restored, no ghost. DM opens another tab and comes back → "Reconnecting…" briefly, then live again, all players still connected. Crashed player who never returns → their entry disappears from the party panel within ~30s.

### Fix #2 — Spectator mode actually works

The bug: spectators got `myClaim.spectator = true` but `pc = null` and `familiars = []`. With no owned entities, the visibility filter computed zero vision sources and the vision mask stripped every token at night. Spectators saw a black canvas.

**Fix:** new DM-controlled `state.spectatorVisionMode`:

- **`'lights'`** — spectators see what visible light sources illuminate. Cinematic — they're an outside observer who sees what light reveals.
- **`'all'`** — spectators see what the whole party collectively sees (every PC's darkvision range plus every light source). Best for observers who want full battlefield awareness.

Set in **Settings → Spectator Vision** (DM-only segmented control).

Implementation:
- `filterStateForPlayer` detects spectators (`claim.spectator && !pc && !familiars.length`), synthesizes a `visionAugmentIds` set based on the mode, uses it in place of `ownedIds` for the vision cutoff. Spectators on dark maps now see the full lit area.
- Player-side render: new `visionSourceIds` memo (spectator-aware) replaces `ownedEntityIds` as the input to `computePlayerVisionSources` and `visionOwned`. The vision mask correctly punches holes for the synthesized sources.
- The spectator still cannot move tokens — `ownedEntityIds` remains empty, so the per-token `canDrag` check still fails for them.

### Fix #3 — Resizable + collapsible sidebars (bestiary, party, revealed)

New `<ResizableSidebar>` wrapper with:

- **Drag-handle** on the inner edge (right edge for left sidebars, left edge for right sidebars). Pointer-events-only, touch + mouse both work. While dragging, the body gets a `resizing-sidebar` class that suppresses the width transition for 1:1 cursor tracking.
- **Collapse button** in the inner top corner. Toggles a `collapsed` class that shrinks the panel to 24px showing only the expand arrow. The whole collapsed strip is clickable to expand (improves discoverability).
- **Per-id persistence** via localStorage (`sidebar-w-<id>`, `sidebar-c-<id>`). Width and collapse state survive reload.

Three sidebars wrapped: DM bestiary, player party, player revealed.

`PartySidebar` and `RevealedMonstersSidebar` were refactored to render content as a fragment instead of their own outer `<div className="sidebar">` — `ResizableSidebar` now provides the wrapper.

CSS: `.main` switched from `display: grid` with fixed column widths to `display: flex` so each `ResizableSidebar` can drive its own width via inline style. The canvas container takes remaining space via `flex: 1`. 17 new CSS rules covering `.resizable-sidebar`, `.sidebar-collapse-btn`, `.sidebar-resize-handle`, hover + active states, body cursor while resizing.

### Fix #4 — Rest menu (long + short rest)

New `RestMenu` dropdown component replaces the standalone Long Rest button.

- **Long Rest** (existing) — full HP, clear most conditions, reset death saves.
- **Short Rest** (new) — heal 1/4 max HP per character (capped at max), clear short-term conditions only (Stunned, Prone, Grappled, Incapacitated, Frightened), reset death saves. Honors D&D 5e "you don't auto-recover full HP from a short rest" by giving a small predictable nudge — the DM can still hand-edit HP for precise hit-die tracking.

New `SHORT_REST` reducer action; `shortRestAll` and `shortRestOne` helpers.

### Fix #5 — Import / Export moved to Settings

The DM topbar had ⇩ Export and ⇧ Import buttons. Both moved to Settings → Session Data.

Settings modal extended with two new optional sections (each gated on prop presence so the same modal works in DM and player modes):

- **Spectator Vision** (DM-only) — segmented picker for `'lights'` vs `'all'`.
- **Session Data** (DM-only) — Export to JSON / Import from JSON buttons.

The topbar is noticeably less crowded.

### Fix #6 — Push View promoted to its own panel

Previously bundled inside the World panel alongside time-of-day and block zones. Extracted to a dedicated `PushViewPanel` with its own toolbar button (`⚑ Push`).

The new panel has Global and Per-Player sections — same controls as before, just standalone. The World panel keeps time-of-day and block zones; tooltip updated.

### Fix #7 — Tools toolbar redesign

The consolidated `🧰 Tools ▾` dropdown is replaced with explicit buttons for each category:

| Old (consolidated) | New (individual button) |
|---|---|
| Tools → Measure | `↔ Measure ▾` (small dropdown for Line / Radius) |
| Tools → Draw | `✎ Draw` |
| Tools → Shapes & Areas | `◧ Shapes ▾` (small dropdown for Rect / Free / Circle / Erase + Hazards) |
| Tools → Encounter (groups) | `⋱ Encounter` |
| Tools → Dice | `🎲 Dice` |
| Tools → Soundboard | `🔊 Sounds` |
| Tools → Reminder | `⚐ Reminder` |

Measure and Shapes keep small dropdowns because they have multiple internal modes; the rest are simple toggles.

The old `<ToolsMenu>` component is no longer rendered (kept in source as dead code for now in case we want to restore the consolidated layout).

Player toolbar gets the same individual-button treatment for its four applicable tools (Measure, Draw, Dice, Reminder).

### Files (6, unchanged manifest)

| File | Size | Notes |
|---|---|---|
| `index.html` | 105 KB | **622 CSS rules** (+16 for ResizableSidebar) |
| `app.compiled.js` | 495 KB | rebuilt with all v7.6 changes |
| `app.js` | ~470 KB | new components + reducers + filter rewrite |
| `README.md` | this changelog appended |
| `build.sh` | unchanged | |
| `.nojekyll` | unchanged | |
...
| Field | Type | Purpose |
|---|---|---|
| `state.spectatorVisionMode` | `'lights'` \| `'all'` | what spectators see at night |
| `claims[peerId].lastSeenTs` | number | for ghost-sweep |

### Reducer additions

- `SPECTATOR_VISION_SET` — DM control
- `CLAIMS_GHOST_SWEEP` — periodic cleanup
- `SHORT_REST` — short-rest semantics

### Tests

- 19/19 sanity checks pass (covering all new components, reducers, state fields, and connection-resilience hooks).
- Earlier release tests preserved: 23/23 dice, 21/21 group-reducer.

### Deploy

Drop the 6 files at the repo root, push to main, hard-refresh.

- No schema break. v7.5 sessions upgrade with `spectatorVisionMode` defaulting to `'lights'`.
- Client compatibility: a v7.5 player connecting to a v7.6 DM works — they receive `tokenGroups: {}` from the filter (unchanged) and don't know about the new fields. A v7.6 spectator connecting to a v7.5 DM falls back to the old broken behavior because the DM doesn't send `spectatorVisionMode` — but the v7.6 client defaults the field to `'lights'` if missing, so it tries (best-effort) to render lights based on entity data alone.

### v7.6 scorecard

| # | Request | Status |
|---|---|---|
| 1 | Player reload doesn't error / leave ghosts | ✅ auto-reconnect + ghost-sweep |
| 2 | DM tab-blur doesn't kill the room | ✅ visibility-return reconnect |
| 3 | Sidebars resizable + collapsible | ✅ ResizableSidebar with persistence |
| 4 | Import/Export in settings | ✅ moved from topbar |
| 5 | Each Tools sub-menu its own button | ✅ 7 individual buttons |
| 6 | Long + short rest in a rest menu | ✅ new RestMenu + SHORT_REST reducer |
| 7 | Push View its own button | ✅ extracted to PushViewPanel |
| 8 | Spectator sees the map | ✅ visionAugmentIds + DM-controlled scope |
| 9 | Spectator vision DM setting | ✅ Settings → Spectator Vision (lights / all) |
